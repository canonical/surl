...
def _get_search_results(config):
    ...
    url = (
        ...
        "date_published,last_updated,sections".format(
...
def _get_snap_metrics(filters, config):
    ...
    for partition in _make_partition(filters, 400):
        ...
        r = surl.store_request(
            config, method="post", url=url, json=payload, headers=headers
        )
        r.raise_for_status()
...
def main():
    parser = argparse.ArgumentParser(description="Snap store operations ...")
